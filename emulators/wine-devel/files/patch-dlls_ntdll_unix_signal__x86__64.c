...
+++ dlls/ntdll/unix/signal_x86_64.c	2024-01-31 20:28:38.135291000 +0300
...
+#if defined(__linux__) || defined(__FreeBSD__)
 ...
 {
     unsigned long rsp;
@@ -824,6 +826,18 @@ static inline ucontext_t *init_handler( void *sigconte
...
+  if (fs != 0 && fs != GSEL(GUFS32_SEL, SEL_UPL))
+  {
...
+  else
+  {
+      assert(rdfsbase() == ((struct amd64_thread_data *)thread_data->cpu_data)->pthread_teb); // ?
+  }
 #endif
     return sigcontext;
 }
@@ -837,6 +851,13 @@ static inline void leave_handler( ucontext_t *sigconte
 #ifdef __linux__
...
@@ -1598,7 +1619,7 @@ __ASM_GLOBAL_FUNC( call_user_mode_callback,
...
@@ -2174,6 +2195,7 @@ static void usr1_handler( int signal, siginfo_t *sigin
  *           LDT support
  */
...
@@ -2231,6 +2253,16 @@ static void ldt_set_entry( WORD sel, LDT_ENTRY entry )
...
+    }
     ...
     exit(1);
@@ -2378,7 +2410,6 @@ static void *mac_thread_gsbase(void)
 }
 #endif
...
-
  ...
  *		signal_init_process
  */
@@ -2439,6 +2470,18 @@ void signal_init_process(void)
             break;
         }
     }
+#elif defined(__FreeBSD__)
...
@@ -2484,8 +2527,9 @@ void call_init_thunk( LPTHREAD_START_ROUTINE entry, vo
...
-
  ...
  */
@@ -2632,7 +2675,11 @@ __ASM_GLOBAL_FUNC( __wine_syscall_dispatcher,
...
+#endif
                    ...
                    "jz 2f\n\t"
@@ -2696,6 +2743,23 @@ __ASM_GLOBAL_FUNC( __wine_syscall_dispatcher,
                    "leaq -0x98(%rbp),%rcx\n"
                    "2:\n\t"
 #endif
+#ifdef __FreeBSD__
...
+#endif
                    ...
                    "movq 0x18(%rcx),%r11\n\t"      /* 2nd argument */
                    "movl %eax,%ebx\n\t"
@@ -2768,7 +2832,7 @@ __ASM_GLOBAL_FUNC( __wine_syscall_dispatcher,
                    "movq 0x20(%rcx),%rsi\n\t"
                    "movq 0x08(%rcx),%rbx\n\t"
...
+#if defined(__linux__) || defined(__FreeBSD__)
                    ...
                    "jz 1f\n\t"
                    "movw %gs:0x338,%fs\n"          /* amd64_thread_data()->fs */
@@ -2902,6 +2966,20 @@ __ASM_GLOBAL_FUNC( __wine_unix_call_dispatcher,
                    "syscall\n\t"
                    "2:\n\t"
 #endif
+#ifdef __FreeBSD__
...
+#endif
                    ...
                    "callq *(%r10,%rdx,8)\n\t"
                    "movq %rsp,%rcx\n\t"
@@ -2920,7 +2998,7 @@ __ASM_GLOBAL_FUNC( __wine_unix_call_dispatcher,
                    /* switch to user stack */
                    "movq 0x88(%rcx),%rsp\n\t"
